...
def quiz4(dp: Dispatcher, bot: Bot):

    class Cappuccin(StatesGroup):
        tempering = State()
        before = State()
        strait = State()
        action = State()
    ...
    async def cmd_start(message: types.Message):
        ...
        await Cappuccin.tempering.set()

    @dp.message_handler(state=Cappuccin.tempering)
    async def process_name(message: types.Message, state: FSMContext):
        await state.update_data(tempering=message.text)
        await message.answer("Пролив перед приготовлением.")
        await message.answer("Балл 1 или 0?")
        await Cappuccin.next()

    @dp.message_handler(state=Cappuccin.before)
    ...
    @dp.message_handler(state=Cappuccin.strait)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.action)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.pitcher)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.pair)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.nozzle)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.transmission)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.remainder)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.temperature)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.foam)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.drink)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.corresponds)
    async def process_age(message: types.Message, state: FSMContext):
    ...
    @dp.message_handler(state=Cappuccin.observed)
